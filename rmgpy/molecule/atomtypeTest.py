...
class TestGetAtomType(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mol24 = Molecule().fromAdjacencyList('''1 S u0 p1 c0 {2,S} {5,S} {7,S} {8,S}
        ...
        self.mol25 = Molecule().fromAdjacencyList('''1 O u0 p3 c-1 {2,S}
                                                     2 S u0 p1 c+1 {1,S} {3,D}
                                                     3 O u0 p2 c0 {2,D}''')

        self.mol26 = Molecule().fromAdjacencyList('''1 S u0 p1 c0 {2,B} {5,B}
        ...
        self.mol27 = Molecule().fromAdjacencyList('''1  O u0 p2 c0 {2,D}
        ...
        self.mol28 = Molecule().fromAdjacencyList('''1 C u0 p1 c-1 {2,T}
                                                     2 S u0 p1 c+1 {1,T}''')

        self.mol29 = Molecule().fromAdjacencyList('''1 S u0 p0 c0 {2,D} {3,S} {4,S} {5,S} {6,S}
                                                     2 O u0 p2 c0 {1,D}
                                                     3 H u0 p0 c0 {1,S}
                                                     4 H u0 p0 c0 {1,S}
                                                     5 H u0 p0 c0 {1,S}
        ...
        self.mol30 = Molecule().fromAdjacencyList('''1 O u0 p3 c-1 {2,S}
        ...
        self.mol33 = Molecule().fromAdjacencyList('''1 S u0 p0 c0 {2,T} {3,S} {4,S} {5,S}
                                                     2 N u0 p1 c0 {1,T}
                                                     3 H u0 p0 c0 {1,S}
                                                     4 H u0 p0 c0 {1,S}
                                                     5 H u0 p0 c0 {1,S}''')

        self.mol34 = Molecule().fromAdjacencyList('''1 S u0 p0 c0 {2,T} {3,D} {4,S}
                                                     2 N u0 p1 c0 {1,T}
                                                     3 O u0 p2 c0 {1,D}
                                                     4 H u0 p0 c0 {1,S}''')

        self.mol35 = Molecule().fromAdjacencyList('''1 N u0 p1 c0 {2,T}
                                                     2 S u0 p0 c0 {1,T} {3,T}
                                                     3 N u0 p1 c0 {2,T}''')

        self.mol36 = Molecule().fromSMILES('O=S=O')
    ...
    def testSulfurTypes(self):
        ...
        self.assertEqual(self.atomType(self.mol24, 0), 'S4s')
        self.assertEqual(self.atomType(self.mol23, 1), 'S4sc')
        self.assertEqual(self.atomType(self.mol24, 2), 'S4d')
        self.assertEqual(self.atomType(self.mol25, 1), 'S4dc')
        self.assertEqual(self.atomType(self.mol26, 0), 'S4b')
        self.assertEqual(self.atomType(self.mol27, 1), 'S4dd')
        self.assertEqual(self.atomType(self.mol36, 1), 'S4dd')
        self.assertEqual(self.atomType(self.mol27, 4), 'S4t')
        self.assertEqual(self.atomType(self.mol28, 1), 'S4tc')
        self.assertEqual(self.atomType(self.mol27, 5), 'S6s')
        self.assertEqual(self.atomType(self.mol29, 0), 'S6d')
        self.assertEqual(self.atomType(self.mol30, 1), 'S6dc')
        self.assertEqual(self.atomType(self.mol31, 1), 'S6dd')
        self.assertEqual(self.atomType(self.mol32, 1), 'S6ddd')
        self.assertEqual(self.atomType(self.mol33, 0), 'S6t')
        self.assertEqual(self.atomType(self.mol34, 0), 'S6td')
        self.assertEqual(self.atomType(self.mol35, 1), 'S6tt')
